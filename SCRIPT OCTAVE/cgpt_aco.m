...
function best_route = ant_colony_optimization(dist_matrix, num_ants, num_iterations, alpha, beta, evaporation_rate, q, x, y)
  ...
  for iteration = 1:num_iterations
    ...
    for ant = 1:num_ants
      ...
      % Menyimpan rute dan panjang rute
      routes(ant, :) = route;
      route_lengths(ant) = calculate_route_length(route, dist_matrix);

      % Memperbarui rute terbaik jika ditemukan rute yang lebih baik
      if route_lengths(ant) < best_route_length
        best_route_length = route_lengths(ant);
        best_route = route;
      end
    end
    ...
    fprintf('Iteration %d: Best route length = %.2f\n', iteration, best_route_length);
  end
